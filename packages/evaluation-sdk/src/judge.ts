...
export async function runJudge(input: JudgeInput, request: JudgeRequest): Promise<JudgeResult> {
  ...
  const raw = await request(prompt, SYSTEM_PROMPT, 1800);
  ...
  if (diagramResult.status !== "generated") {
    if (!hasExplicitDiagramEvidence(input.readme)) {
      ...
      if (explicitSequence) {
        ...
        try {
          const recoveryRaw = await request(buildDiagramRecoveryPrompt(input), SYSTEM_PROMPT, 900);
...
export type JudgeRequest = ((prompt: string, system: string, maxTokens: number) => Promise<unknown>) & { model: string };
