...
- GitHub 项目按实际交付物分类：课程、教程、示例集合与 SDK 不会仅因出现 “MCP” 就被当成 MCP Server；报告记录本次评测类型与分类器版本。
...
- 运营后台和 `npm run growth:report` 按 D1/D7/D30 展示访问、注册、评测、免费额度与订阅数据，不用估算值代替真实数据。
  - 百分比分母为 0、数据缺失或计数无效时，JSON 比率字段返回 `null`，CLI / 后台显示“暂无有效样本”/“暂无样本”；只有存在样本且分子为 0 才显示 `0%`。消费者应先检查数据可用标记和样本数，勿将 `null` 当作 0。重复点击比率可超过 100%，不会静默封顶。
  - 流量按北京时间自然日汇总（D7 含今天和此前 6 天）；注册、任务与新增项目为滚动时间窗口。两者不可当作完全同一时间口径。升级入口尚未实现，`upgradeTrackingAvailable: false` 表示不可测，不表示测得 0 次。
...
分类器升级后先运行 `npm run types:reclassify` 查看汇总迁移量。脚本会用 GitHub 当前公开元数据复核迁移候选，默认拒绝一次修改超过 20 个项目，且只输出聚合信息；确认在安全上限内后运行 `npm run types:reclassify -- --execute`。它仅更新公开 GitHub 项目的类型，并为已有报告创建强制刷新任务；活跃任务由数据库唯一索引避免重复。任一候选无法复核时整批失败，不使用过期元数据猜测。
...
本项目采用 [Apache License 2.0](LICENSE) 开源。使用、修改和分发本项目时，请遵守许可证中的署名、变更声明及专利条款。
